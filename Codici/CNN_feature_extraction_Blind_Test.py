...
dataset_folder = 'Blind_Test_Dataset'
# Process the complete dataset folder with the provided Excel file
excel_filename = dataset_folder + '/Blind_Test_Dataset.xlsx'
...
def extract_features_in_batches(image_paths, batch_size=32):
    ...
    print(
        f'Feature extraction completed in {end_time - start_time:.2f} seconds, averaging {avg_time_per_image:.4f} seconds per image.')
...
def process_complete_dataset(dataset_folder, excel_filename, batch_size=32):
    ...
    # Combine with crop data (ignoring 'RST')
    plot_info = df.set_index('Plot')[['Crop']].to_dict('index')

    # Create a final dataset
    final_data = []
    for plot, features in plot_features.items():
        plot_info_entry = plot_info.get(int(plot), None)
        if plot_info_entry is not None:
            final_data.append((plot, features, plot_info_entry['Crop']))
...
final_df = process_complete_dataset(dataset_folder, excel_filename, batch_size=32)
